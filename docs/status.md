...
- **Phase14-PR-1**: pytest設定の単一真実化（pytest.ini）を再発防止まで固定。`tests/test_release_readiness.py` に `pyproject.toml` をTOMLとして検査し `tool.pytest.ini_options` の再導入を検知して失敗させるガードテストを実装し、`pytest.ini` の存在と `[pytest]` セクションも合わせて検証する運用へ強化。
