...
> `requirements.txt` / `requirements-dev.txt` は clone 済み checkout 用の repo-local convenience wrappers です。外部利用者向けの依存 truth source は `pyproject.toml` なので、配布物から使う場合は `pip install "po-core-flyingpig==1.0.3"` か extras を使ってください（repository target: `1.1.1`; `1.1.0` は TestPyPI-only・superseded; `1.1.1` publish は pending）。
